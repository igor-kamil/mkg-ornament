    ...
    <div class="fixed inset-0 z-50 flex justify-center items-center" v-if="visible">
        ...
        <div class="relative mx-4 bg-white w-full max-w-2xl bg-white rounded-xl">
            ...
            <div class="px-4 py-6">
                ...
                <a :href="item.web_url" target="_blank" class="bg-gray-softest border-2 border-black p-2.5 rounded-xl block" v-if="item.web_url">
                ...
                <ConfirmButton class="bg-black text-white my-4" @click="emit('close')">{{ $t('Close') }}</ConfirmButton>
            </div>
        </div>
    </div>
</template>
